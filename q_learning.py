...
class Game:
    ...
    def reset(self):
        ...
        self.board = Board(grid=np.ones((N,N))*np.nan)
...
class Board:
    def __init__(self, grid=np.ones((N,N))*np.nan):
        self.grid = grid

    def winner(self):
        rows = [self.grid[i,:] for i in range(N)]
        cols = [self.grid[:,j] for j in range(N)]
        diag = [np.array([self.grid[i,i] for i in range(N)])]
        cross_diag = [np.array([self.grid[(N-1)-i,i] for i in range(N)])]
        ...
        if any_lane(np.ones(N)):
            return "X"
        elif any_lane(np.zeros(N)):
            return "O"
    ...
    def available_moves(self):
        return [(i,j) for i in range(N) for j in range(N) if np.isnan(self.grid[i][j])]
